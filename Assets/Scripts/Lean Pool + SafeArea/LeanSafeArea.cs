...
namespace Lean.Gui
{
    ...
    public class LeanSafeArea : UIBehaviour
    {
        ...
        public void UpdateSafeArea()
        {
            ...
            if (IsResize)
            {
                var min = safeMin.x / screenW;
                var max = safeMax.x / screenW;
                if (Screen.orientation == ScreenOrientation.LandscapeRight)
                {
                    cachedRectTransform.anchorMin = new Vector2(0, safeMin.y / screenH);
                    cachedRectTransform.anchorMax = new Vector2(max, safeMax.y / screenH);
                }
                else if(Screen.orientation == ScreenOrientation.LandscapeLeft)
                {
                    cachedRectTransform.anchorMin = new Vector2(safeMin.x / screenW, safeMin.y / screenH);
                    cachedRectTransform.anchorMax = new Vector2(1, safeMax.y / screenH);
                }
                else
                {
